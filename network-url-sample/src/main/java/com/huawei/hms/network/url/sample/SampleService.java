...
public interface SampleService {
     ...
     * (1) @GET, @Query, and @QueryMap are used together. In non-GET request, @FormUrlEncoded, @ FieldMap, and @ Field are used together.
     * (2) @GET cannot contain @Body annotation.
     * (3) The @ ("value") and @Url request modes cannot coexist. The @ request mode includes @GET, @POST, @PUT, @HEAD, and @DELETE. This mode is used to set the URL, which means that the URL has been dynamically configured. Therefore, @Url is not required in this case.
     * (4) @Url and @Path are mutually exclusive and cannot coexist. An exception will be thrown if they coexist.
     * (5) @Url must be used before @Query, @QueryMap, or @QueryName.
     * (6) The parameter of @xxxMap must be a subclass of the Map API.
     */
